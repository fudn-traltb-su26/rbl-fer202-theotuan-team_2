...
-Nguyễn Văn Đức (Trưởng nhóm — Kỹ thuật & Đồng bộ State):
Chịu trách nhiệm thiết lập luồng dữ liệu trung gian, xử lý các trạng thái phụ thuộc (Derived State) và đồng bộ tín hiệu giữa các linh kiện con.

Nhiệm vụ cụ thể: Trực tiếp cấu hình luồng dữ liệu tại App.jsx; tính toán Derived State filteredBooks từ các bộ lọc mà không cần tạo thêm state dư thừa; truyền hàm callback onSearch xuống SearchBar và truyền dữ liệu đã lọc xuống BookGrid (Lifting State Up); phối hợp với Thành viên Y để tích hợp và kiểm thử toàn bộ tương tác trên giao diện. (Kế thừa phần việc của thành viên E).

-Chu Cao Huy (Thành viên — Logic & State Management):
Chịu trách nhiệm quản lý kho trạng thái cốt lõi của ứng dụng (Giỏ hàng) và giải trình lý thuyết kỹ thuật.

Nhiệm vụ cụ thể: Khai báo và xử lý cart state trong App.jsx; xây dựng hàm handleAddToCart đảm bảo cập nhật mảng một cách bất biến (sử dụng toán tử spread [...] hoặc .map(), tuyệt đối không dùng .push()); tính toán tổng số lượng item (totalItems); biên soạn nội dung báo cáo giải thích bản chất và lý do phải sử dụng immutable update trong React. (Kế thừa phần việc của thành viên B và một phần của thành viên E).

-Nguyễn Kim Anh Duy (Thành viên — UI Tương tác & Controlled Component):
Chịu trách nhiệm xây dựng các linh kiện thu thập dữ liệu từ người dùng và bộ lọc hiển thị.

Nhiệm vụ cụ thể: Phát triển linh kiện SearchBar.jsx thành một Controlled Component hoàn chỉnh (quản lý keyword state, ràng buộc dữ liệu đầu vào, xử lý validation tối thiểu 2 ký tự và tính năng Clear nhanh nút ×); hiện thực hóa tính năng lọc theo danh mục bằng cách thiết lập state activeCategory và xử lý đổi style active cho các nút bấm danh mục. (Kế thừa phần việc của thành viên A, C và một phần của thành viên E).
